...
class DesugarUnixUriUtils {
    ...
    private static boolean match(char c, long lowMask, long highMask) {
        return c < '@' ? (lowMask & (1 << c)) != 0 : c < 128 && ((1 << (c - 64)) & highMask) != 0;
    }
    ...
    static DesugarUnixPath fromUri(FileSystem fs, URI uri, String userDir, String rootDir) {
        ...
        return new DesugarUnixPath(fs, new String(bArr, DesugarUtil.jnuEncoding()), userDir, rootDir);
    }

    static URI toUri(DesugarUnixPath up) {
        byte[] asByteArray = up.toAbsolutePath().asByteArray();
        ...
        if (sb.charAt(sb.length() - 1) != '/' && up.toFile().isDirectory()) {
            sb.append('/');
        }
        try {
    ...
    private static long lowMask(char first, char last) {
        long j = 0;
        for (int max = Math.max(Math.min(first, 63), 0); max <= Math.max(Math.min(last, 63), 0); max++) {
            j |= 1 << max;
        }
        return j;
    }
    ...
    private static long highMask(char first, char last) {
        long j = 0;
        for (int max = Math.max(Math.min(first, 127), 64) - 64; max <= Math.max(Math.min(last, 127), 64) - 64; max++) {
            j |= 1 << max;
        }
        return j;
    }

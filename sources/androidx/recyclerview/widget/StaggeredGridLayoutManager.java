...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private final Runnable mCheckForGapsRunnable = new 1();
    ...
    View hasGapsToFix() {
        ...
        for (int i = firstChildIndex; i != childLimit; i += nextChildDiff) {
            ...
            if (!lp.mFullSpan && i + nextChildDiff != childLimit) {
                ...
                if (compareSpans) {
                    LayoutParams nextLp = nextChild.getLayoutParams();
    ...
    private boolean checkSpanForGap(Span span) {
        if (this.mShouldReverseLayout) {
            if (span.getEndLine() < this.mPrimaryOrientation.getEndAfterPadding()) {
                View endView = (View) span.mViews.get(span.mViews.size() - 1);
                LayoutParams lp = span.getLayoutParams(endView);
                return !lp.mFullSpan;
            }
        } else if (span.getStartLine() > this.mPrimaryOrientation.getStartAfterPadding()) {
            View startView = (View) span.mViews.get(0);
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        LayoutParams prevFocusLayoutParams = directChild.getLayoutParams();
    ...
    static class LazySpanLookup {
        ...
        private int invalidateFullSpansAfter(int position) {
            ...
            FullSpanItem fsi2 = (FullSpanItem) this.mFullSpanItems.get(nextFsiIndex);

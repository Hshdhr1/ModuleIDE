...
class OpReorderer {
    ...
    void reorderOps(List list) {
        while (true) {
            int badMove = getLastMoveOutOfOrder(list);
            if (badMove != -1) {
                swapMoveOp(list, badMove, badMove + 1);
            } else {
                return;
            }
        }
    ...
    private void swapMoveOp(List list, int badMove, int next) {
        AdapterHelper.UpdateOp moveOp = (AdapterHelper.UpdateOp) list.get(badMove);
        AdapterHelper.UpdateOp nextOp = (AdapterHelper.UpdateOp) list.get(next);
        switch (nextOp.cmd) {
            case 1:
    ...
    void swapMoveRemove(List list, int movePos, AdapterHelper.UpdateOp moveOp, int removePos, AdapterHelper.UpdateOp removeOp) {
    ...
    private void swapMoveAdd(List list, int move, AdapterHelper.UpdateOp moveOp, int add, AdapterHelper.UpdateOp addOp) {
        int offset = 0;
    ...
    void swapMoveUpdate(List list, int move, AdapterHelper.UpdateOp moveOp, int update, AdapterHelper.UpdateOp updateOp) {
    ...
    private int getLastMoveOutOfOrder(List list) {
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            AdapterHelper.UpdateOp op1 = (AdapterHelper.UpdateOp) list.get(i);
            if (op1.cmd == 8) {
                if (foundNonMove) {
                    return i;
                }

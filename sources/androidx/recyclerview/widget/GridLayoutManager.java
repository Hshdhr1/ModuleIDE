...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler, RecyclerView.State state, View host, AccessibilityNodeInfoCompat info) {
        LayoutParams layoutParams = host.getLayoutParams();
        if (!(layoutParams instanceof LayoutParams)) {
            super.onInitializeAccessibilityNodeInfoForItem(host, info);
            return;
        }
        LayoutParams glp = layoutParams;
    ...
    View findReferenceChild(RecyclerView.Recycler recycler, RecyclerView.State state, boolean layoutFromEnd, boolean traverseChildrenInReverseOrder) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                ...
                if (span != 0) {
                    continue;
                } else if (view.getLayoutParams().isItemRemoved()) {
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult result) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            float otherSize = (1.0f * this.mOrientationHelper.getDecoratedMeasurementInOther(view2)) / view2.getLayoutParams().mSpanSize;
        ...
        for (int i3 = 0; i3 < count; i3++) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view4) != maxSize) {
                LayoutParams lp = view4.getLayoutParams();
        ...
        for (int i4 = 0; i4 < count; i4++) {
            ...
            LayoutParams params = view5.getLayoutParams();
            if (this.mOrientation == 1) {
                if (isLayoutRTL()) {
    ...
    public View onFocusSearchFailed(View focused, int focusDirection, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        LayoutParams lp = prevFocusedChild.getLayoutParams();
        ...
        for (int i = start; i != limit; i += inc) {
            ...
            if (candidate.hasFocusable() && spanGroupIndex != focusableSpanGroupIndex) {
                ...
                LayoutParams candidateLp = candidate.getLayoutParams();

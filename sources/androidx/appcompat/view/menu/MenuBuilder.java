...
public class MenuBuilder implements SupportMenu {
    ...
    private static int findInsertIndex(ArrayList arrayList, int ordering) {
    ...
    void findItemsWithShortcutForKey(List list, int keyCode, KeyEvent event) {
        ...
        for (int i = 0; i < N; i++) {
            MenuItemImpl item = (MenuItemImpl) this.mItems.get(i);
            if (item.hasSubMenu()) {
                item.getSubMenu().findItemsWithShortcutForKey(list, keyCode, event);
            }

...
public final class ResourceManagerInternal {
    ...
    public synchronized void onConfigurationChanged(Context context) {
        LongSparseArray<WeakReference<Drawable.ConstantState>> cache = (LongSparseArray) this.mDrawableCaches.get(context);
        if (cache != null) {
            cache.clear();
        }
    }
    ...
    private ColorStateList getTintListFromCache(Context context, int resId) {
        ...
        if (weakHashMap == null || (tints = (SparseArrayCompat) weakHashMap.get(context)) == null) {
            return null;
        }
    ...
    static class DrawableDelegate implements InflateDelegate {
        ...
        public Drawable createFromXmlInner(Context context, XmlPullParser parser, AttributeSet attrs, Resources.Theme theme) {
            ...
            try {
                Class<? extends Drawable> drawableClass = DrawableDelegate.class.getClassLoader().loadClass(className).asSubclass(Drawable.class);
                Drawable drawable = (Drawable) drawableClass.getDeclaredConstructor(new Class[0]).newInstance(new Object[0]);

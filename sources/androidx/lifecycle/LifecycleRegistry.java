...
public class LifecycleRegistry extends Lifecycle {
    ...
    private Lifecycle.State calculateTargetState(LifecycleObserver observer) {
        ...
        if (!this.mParentStates.isEmpty()) {
            parentState = (Lifecycle.State) this.mParentStates.get(r1.size() - 1);
        }
    ...
    public void addObserver(LifecycleObserver observer) {
        ...
        if (previous != null || (lifecycleOwner = (LifecycleOwner) this.mLifecycleOwner.get()) == null) {
            return;
        }
    ...
    private void forwardPass(LifecycleOwner lifecycleOwner) {
        ...
        while (iteratorWithAdditions.hasNext() && !this.mNewEventOccurred) {
            Map.Entry<LifecycleObserver, ObserverWithState> entry = (Map.Entry) iteratorWithAdditions.next();

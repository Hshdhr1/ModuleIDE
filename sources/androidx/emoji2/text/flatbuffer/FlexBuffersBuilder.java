...
public class FlexBuffersBuilder {
    ...
    static /* synthetic */ ReadWriteBuf access$000(FlexBuffersBuilder x0) {
        return x0.bb;
    }
    ...
    class 1 implements Comparator {
        ...
        public int compare(Value o1, Value o2) {
            ...
            int io = o2.key;
            do {
                c1 = FlexBuffersBuilder.access$000(FlexBuffersBuilder.this).get(ia);
                c2 = FlexBuffersBuilder.access$000(FlexBuffersBuilder.this).get(io);
                if (c1 == 0) {
                    return c1 - c2;
                }
                ia++;
    ...
    private int align(int alignment) {
        ...
        int padBytes = Value.access$100(this.bb.writePosition(), byteWidth);
        while (true) {
    ...
    public ByteBuffer finish() {
        int byteWidth = align(Value.access$200((Value) this.stack.get(0), this.bb.writePosition(), 0));
        writeAny((Value) this.stack.get(0), byteWidth);
        this.bb.put(Value.access$300((Value) this.stack.get(0)));
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        int prefixElems = 1;
        if (keys != null) {
            bitWidth = Math.max(bitWidth, Value.access$200(keys, this.bb.writePosition(), 0));
            prefixElems = 1 + 2;
        }
        int vectorType = 4;
        for (int i2 = start; i2 < this.stack.size(); i2++) {
            int elemWidth = Value.access$200((Value) this.stack.get(i2), this.bb.writePosition(), i2 + prefixElems);
            bitWidth = Math.max(bitWidth, elemWidth);
            if (typed && i2 == start) {
                vectorType = ((Value) this.stack.get(i2)).type;
        ...
        for (int i3 = start; i3 < this.stack.size(); i3++) {
            writeAny((Value) this.stack.get(i3), byteWidth);
        }
        if (!typed) {
            for (int i4 = start; i4 < this.stack.size(); i4++) {
                this.bb.put(Value.access$400((Value) this.stack.get(i4), bitWidth));
            }
        }
        if (keys != null) {
            i = 9;
    ...
    private Value createKeyVector(int start, int length) {
        ...
        for (int i = start; i < this.stack.size(); i++) {
            int elemWidth = Value.access$500(4, 0, ((Value) this.stack.get(i)).key, this.bb.writePosition(), i + 1);
            bitWidth = Math.max(bitWidth, elemWidth);
        }
        int byteWidth = align(bitWidth);
        writeInt(length, byteWidth);
        int vloc = this.bb.writePosition();
        for (int i2 = start; i2 < this.stack.size(); i2++) {
            int i3 = ((Value) this.stack.get(i2)).key;
            writeOffset(((Value) this.stack.get(i2)).key, byteWidth);
        }
    ...
    private static class Value {
        ...
        static /* synthetic */ int access$100(int x0, int x1) {
            return paddingBytes(x0, x1);
        }

        static /* synthetic */ int access$200(Value x0, int x1, int x2) {
            return x0.elemWidth(x1, x2);
        }

        static /* synthetic */ byte access$300(Value x0) {
            return x0.storedPackedType();
        }

        static /* synthetic */ byte access$400(Value x0, int x1) {
            return x0.storedPackedType(x1);
        }

        static /* synthetic */ int access$500(int x0, int x1, long x2, int x3, int x4) {
            return elemWidth(x0, x1, x2, x3, x4);
        }

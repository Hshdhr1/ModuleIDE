...
public final class SpannableBuilder extends SpannableStringBuilder {
    ...
    public Object[] getSpans(int queryStart, int queryEnd, Class cls) {
        if (isWatcher(cls)) {
            WatcherWrapper[] spans = (WatcherWrapper[]) super.getSpans(queryStart, queryEnd, WatcherWrapper.class);
            Object[] objArr = (Object[]) Array.newInstance(cls, spans.length);
            for (int i = 0; i < spans.length; i++) {
                objArr[i] = spans[i].mObject;
            }
            return objArr;
        }
    ...
    private WatcherWrapper getWatcherFor(Object object) {
        for (int i = 0; i < this.mWatchers.size(); i++) {
            WatcherWrapper watcher = (WatcherWrapper) this.mWatchers.get(i);

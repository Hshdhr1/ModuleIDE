...
final class EmojiProcessor {
    ...
    CharSequence process(CharSequence charSequence, int start, int end, int maxEmojiCount, boolean replaceAll) {
        ...
        if (charSequence2 != null && (spans = (EmojiSpan[]) charSequence2.getSpans(start, end, EmojiSpan.class)) != null && spans.length > 0) {
            for (EmojiSpan span : spans) {
                int spanStart = charSequence2.getSpanStart(span);
                int spanEnd = charSequence2.getSpanEnd(span);
                if (spanStart != end) {
                    charSequence2.removeSpan(span);
                }
        ...
        if (start != end && start < charSequence.length()) {
            if (maxEmojiCount != Integer.MAX_VALUE && charSequence2 != null) {
                maxEmojiCount -= ((EmojiSpan[]) charSequence2.getSpans(0, charSequence2.length(), EmojiSpan.class)).length;
            }
            int addedCount = 0;
            ProcessorSm sm = new ProcessorSm(this.mMetadataRepo.getRootNode(), this.mUseEmojiAsDefaultStyle, this.mEmojiAsDefaultStyleExceptions);
                ...
                switch (action) {
                    ...
                    case 3:
                        if (replaceAll || !hasGlyph(charSequence, start, currentOffset, sm.getFlushMetadata())) {
                            ...
                            addEmoji(charSequence2, sm.getFlushMetadata(), start, currentOffset);
                            addedCount++;
                        }
                        start = currentOffset;
                        break;
            ...
            if (sm.isInFlushableState() && addedCount < maxEmojiCount && (replaceAll || !hasGlyph(charSequence, start, currentOffset, sm.getCurrentMetadata()))) {
                ...
                addEmoji(charSequence2, sm.getCurrentMetadata(), start, currentOffset);
                int i = addedCount + 1;
            }

...
class MotionConstrainedPoint implements Comparable {
    ...
    void different(MotionConstrainedPoint points, HashSet keySet) {
    ...
    int getCustomDataCount(String name) {
        return ((ConstraintAttribute) this.attributes.get(name)).numberOfInterpolatedValues();
    }
    ...
    public void addValues(HashMap splines, int mFramePosition) {
        for (String s : splines.keySet()) {
            ViewSpline ViewSpline = (ViewSpline) splines.get(s);
            char c = 65535;
            switch (s.hashCode()) {
                case -1249320806:
                ...
                default:
                    if (s.startsWith("CUSTOM")) {
                        ...
                        if (this.attributes.containsKey(customName)) {
                            ConstraintAttribute custom = (ConstraintAttribute) this.attributes.get(customName);
                            if (ViewSpline instanceof ViewSpline.CustomSet) {
                                ...
                                StringBuilder sb = new StringBuilder(String.valueOf(s).length() + 69 + String.valueOf(valueOf).length());
                                sb.append(s);

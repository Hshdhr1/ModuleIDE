...
public class WidgetGroup {
    ...
    private int solverMeasure(LinearSystem system, ArrayList arrayList, int orientation) {
        ConstraintWidgetContainer container = (ConstraintWidgetContainer) ((ConstraintWidget) arrayList.get(0)).getParent();
        system.reset();
        container.addToSolver(system, false);
        for (int i = 0; i < arrayList.size(); i++) {
            ConstraintWidget widget = (ConstraintWidget) arrayList.get(i);
            widget.addToSolver(system, false);
        }
        if (orientation == 0 && container.mHorizontalChainsSize > 0) {
            Chain.applyChainConstraints(container, system, arrayList, 0);
        }
        if (orientation == 1 && container.mVerticalChainsSize > 0) {
            Chain.applyChainConstraints(container, system, arrayList, 1);
        }
        try {
            system.minimize();
        } catch (Exception e) {
        ...
        for (int i2 = 0; i2 < arrayList.size(); i2++) {
            ConstraintWidget widget2 = (ConstraintWidget) arrayList.get(i2);
    ...
    public void apply() {
        ...
        for (int i = 0; i < this.results.size(); i++) {
            MeasureResult result = (MeasureResult) this.results.get(i);
            result.apply();
        }
    }
    ...
    public boolean intersectWith(WidgetGroup group) {
        for (int i = 0; i < this.widgets.size(); i++) {
            ConstraintWidget widget = (ConstraintWidget) this.widgets.get(i);

...
public class ConstraintAnchor {
    ...
    public void findDependents(int orientation, ArrayList arrayList, WidgetGroup group) {
        HashSet hashSet = this.mDependents;
        if (hashSet != null) {
            Iterator it = hashSet.iterator();
            while (it.hasNext()) {
                ConstraintAnchor anchor = (ConstraintAnchor) it.next();
                Grouping.findDependents(anchor.mOwner, orientation, arrayList, group);
            }
        }
    }
    ...
    public void copyFrom(ConstraintAnchor source, HashMap hashMap) {
        HashSet hashSet;
        ...
        if (constraintAnchor2 != null) {
            ...
            ConstraintWidget owner = (ConstraintWidget) hashMap.get(source.mTarget.mOwner);
    ...
    private boolean isConnectionToMe(ConstraintWidget target, HashSet hashSet) {
        ...
        for (int i = 0; i < targetAnchorsSize; i++) {
            ...
            if (anchor.isSimilarDimensionConnection(this) && anchor.isConnected() && isConnectionToMe(anchor.getTarget().getOwner(), hashSet)) {
                return true;
            }
        }
        return false;

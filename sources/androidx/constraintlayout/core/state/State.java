...
public class State {
    ...
    Reference reference(Object key) {
        return (Reference) this.mReferences.get(key);
    }

    public ConstraintReference constraints(Object key) {
        Reference reference = (Reference) this.mReferences.get(key);
        if (reference == null) {
    ...
    public HelperReference helper(Object key, Helper type) {
        ...
        HelperReference reference = (HelperReference) this.mHelperReferences.get(key);
        if (reference == null) {
            switch (1.$SwitchMap$androidx$constraintlayout$core$state$State$Helper[type.ordinal()]) {
                case 1:
    ...
    public void setTag(String key, String tag) {
        ...
        if (ref instanceof ConstraintReference) {
            ...
            if (!this.mTags.containsKey(tag)) {
                ...
                list = (ArrayList) this.mTags.get(tag);
            }
            list.add(key);
        }
    }

    public ArrayList getIdsForTag(String tag) {
        if (this.mTags.containsKey(tag)) {
            return (ArrayList) this.mTags.get(tag);
        }
        return null;
    }
    ...
    public void apply(ConstraintWidgetContainer container) {
        ...
        for (Object key : this.mHelperReferences.keySet()) {
            HelperWidget helperWidget3 = ((HelperReference) this.mHelperReferences.get(key)).getHelperWidget();
            if (helperWidget3 != null) {
                Reference constraintReference = (Reference) this.mReferences.get(key);
        ...
        for (Object key2 : this.mReferences.keySet()) {
            Reference reference = (Reference) this.mReferences.get(key2);
            if (reference != this.mParent && (reference.getFacade() instanceof HelperReference) && (helperWidget2 = ((HelperReference) reference.getFacade()).getHelperWidget()) != null) {
                Reference constraintReference2 = (Reference) this.mReferences.get(key2);
        ...
        Iterator it = this.mReferences.keySet().iterator();
        while (it.hasNext()) {
            Reference reference2 = (Reference) this.mReferences.get(it.next());
        ...
        Iterator it2 = this.mHelperReferences.keySet().iterator();
        while (it2.hasNext()) {
            HelperReference reference3 = (HelperReference) this.mHelperReferences.get(it2.next());
            if (reference3.getHelperWidget() != null) {
                Iterator it3 = reference3.mReferences.iterator();
                while (it3.hasNext()) {
                    reference3.getHelperWidget().add(((Reference) this.mReferences.get(it3.next())).getConstraintWidget());
                }
                reference3.apply();
            } else {
                reference3.apply();
        ...
        Iterator it4 = this.mReferences.keySet().iterator();
        while (it4.hasNext()) {
            Reference reference4 = (Reference) this.mReferences.get(it4.next());
            if (reference4 != this.mParent && (reference4.getFacade() instanceof HelperReference) && (helperWidget = (helperReference = (HelperReference) reference4.getFacade()).getHelperWidget()) != null) {
                Iterator it5 = helperReference.mReferences.iterator();
                while (it5.hasNext()) {
                    Object keyRef = it5.next();
                    Reference constraintReference3 = (Reference) this.mReferences.get(keyRef);
        ...
        for (Object key3 : this.mReferences.keySet()) {
            Reference reference5 = (Reference) this.mReferences.get(key3);
            reference5.apply();

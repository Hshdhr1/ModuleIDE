...
public class ConstraintReference implements Reference {
    ...
    private HashMap mCustomColors = new HashMap();
    private HashMap mCustomFloats = new HashMap();
    ...
    private void applyConnection(ConstraintWidget widget, Object opaqueTarget, State.Constraint type) {
        ...
        int i = 1.$SwitchMap$androidx$constraintlayout$core$state$State$Constraint[type.ordinal()];
        switch (1.$SwitchMap$androidx$constraintlayout$core$state$State$Constraint[type.ordinal()]) {
            case 1:
    ...
    public void apply() {
        ...
        if (hashMap != null) {
            for (String key : hashMap.keySet()) {
                Integer color = (Integer) this.mCustomColors.get(key);
                this.mConstraintWidget.frame.setCustomAttribute(key, 902, color.intValue());
            }
        }
        HashMap hashMap2 = this.mCustomFloats;
        if (hashMap2 != null) {
            for (String key2 : hashMap2.keySet()) {
                float value = ((Float) this.mCustomFloats.get(key2)).floatValue();
                this.mConstraintWidget.frame.setCustomAttribute(key2, 901, value);
            }
        }
    }
}

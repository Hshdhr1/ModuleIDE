...
public class Transition {
    ...
    public KeyPosition findPreviousPosition(String target, int frameNumber) {
        ...
        while (frameNumber >= 0) {
            HashMap<String, KeyPosition> map = (HashMap) this.keyPositions.get(Integer.valueOf(frameNumber));
            if (map != null && (keyPosition = (KeyPosition) map.get(target)) != null) {
                return keyPosition;
            }
            frameNumber--;
        }
    ...
    public int getNumberKeyPositions(WidgetFrame frame) {
        ...
        for (int frameNumber = 0; frameNumber <= 100; frameNumber++) {
            HashMap<String, KeyPosition> map = (HashMap) this.keyPositions.get(Integer.valueOf(frameNumber));
            if (map != null) {
                KeyPosition keyPosition = (KeyPosition) map.get(frame.widget.stringId);
                if (keyPosition != null) {
                    numKeyPositions++;
                }
            }
    ...
    public void fillKeyPositions(WidgetFrame frame, float[] x, float[] y, float[] pos) {
        ...
        for (int frameNumber = 0; frameNumber <= 100; frameNumber++) {
            HashMap<String, KeyPosition> map = (HashMap) this.keyPositions.get(Integer.valueOf(frameNumber));
            if (map != null && (keyPosition = (KeyPosition) map.get(frame.widget.stringId)) != null) {
    ...
    public void setTransitionProperties(TypedBundle bundle) {
        ...
        this.mAutoTransition = bundle.getInteger(704);
    }
    ...
    public void addKeyPosition(String target, int frame, int type, float x, float y) {
        ...
        HashMap<String, KeyPosition> map = (HashMap) this.keyPositions.get(Integer.valueOf(frame));
        if (map == null) {
            map = new HashMap<>();
    ...
    public void interpolate(int parentWidth, int parentHeight, float progress) {
        for (String key : this.state.keySet()) {
            WidgetState widget = (WidgetState) this.state.get(key);
            widget.interpolate(parentWidth, parentHeight, progress, this);
        }
    }

    public WidgetFrame getStart(String id) {
        WidgetState widgetState = (WidgetState) this.state.get(id);
        if (widgetState == null) {
            return null;
        }
    ...
    private WidgetState getWidgetState(String widgetId) {
        return (WidgetState) this.state.get(widgetId);
    }

    private WidgetState getWidgetState(String widgetId, ConstraintWidget child, int transitionState) {
        WidgetState widgetState = (WidgetState) this.state.get(widgetId);
        if (widgetState == null) {

...
class MotionConstrainedPoint implements Comparable {
    ...
    void different(MotionConstrainedPoint points, HashSet hashSet) {
    ...
    int getCustomDataCount(String name) {
        return ((CustomVariable) this.mCustomVariable.get(name)).numberOfInterpolatedValues();
    }

    int getCustomData(String name, double[] value, int offset) {
        CustomVariable a = (CustomVariable) this.mCustomVariable.get(name);
    ...
    public void addValues(HashMap hashMap, int mFramePosition) {
        for (String s : hashMap.keySet()) {
            SplineSet ViewSpline = (SplineSet) hashMap.get(s);
            char c = 65535;
            switch (s.hashCode()) {
                case -1249320806:
                ...
                default:
                    if (!s.startsWith("CUSTOM")) {
                        ...
                        if (this.mCustomVariable.containsKey(customName)) {
                            CustomVariable custom = (CustomVariable) this.mCustomVariable.get(customName);

...
public final class ShareCompat {
    ...
    static String getCallingPackage(Intent intent) {
        String result = intent.getStringExtra("androidx.core.app.EXTRA_CALLING_PACKAGE");
        if (result == null) {
            return intent.getStringExtra("android.support.v4.app.EXTRA_CALLING_PACKAGE");
        }
        return result;
    }
    ...
    static ComponentName getCallingActivity(Intent intent) {
        ComponentName result = intent.getParcelableExtra("androidx.core.app.EXTRA_CALLING_ACTIVITY");
        if (result == null) {
            return intent.getParcelableExtra("android.support.v4.app.EXTRA_CALLING_ACTIVITY");
        }
        return result;
    }
    ...
    public static void configureMenuItem(MenuItem item, IntentBuilder shareIntent) {
        ...
        ShareActionProvider actionProvider = item.getActionProvider();
        if (!(actionProvider instanceof ShareActionProvider)) {
            provider = new ShareActionProvider(shareIntent.getContext());
        } else {
            provider = actionProvider;
        }
        provider.setShareHistoryFileName(".sharecompat_" + shareIntent.getContext().getClass().getName());
    ...
    public static class IntentBuilder {
        ...
        public IntentBuilder(Context launchingContext) {
            ...
            this.mIntent = action;
            action.putExtra("androidx.core.app.EXTRA_CALLING_PACKAGE", launchingContext.getPackageName());
            action.putExtra("android.support.v4.app.EXTRA_CALLING_PACKAGE", launchingContext.getPackageName());
        ...
        private void combineArrayExtra(String extra, ArrayList arrayList) {
            ...
            String[] finalAddresses = new String[arrayList.size() + currentLength];
            arrayList.toArray(finalAddresses);
            if (currentAddresses != null) {
                System.arraycopy(currentAddresses, 0, finalAddresses, arrayList.size(), currentLength);
            }
    ...
    private static class Api16Impl {
        ...
        static void migrateExtraStreamToClipData(Intent intent, ArrayList arrayList) {
            ...
            ClipData clipData = new ClipData((CharSequence) null, new String[]{intent.getType()}, new ClipData.Item(text, htmlText, (Intent) null, (Uri) arrayList.get(0)));

...
class NotificationCompatBuilder implements NotificationBuilderWithBuilderAccessor {
    ...
    NotificationCompatBuilder(NotificationCompat.Builder b) {
        ...
        if (Build.VERSION.SDK_INT >= 19 && Build.VERSION.SDK_INT < 21 && (people2 = combineLists(getPeople(b.mPersonList), b.mPeople)) != null && !people2.isEmpty()) {
            this.mExtras.putStringArray("android.people", (String[]) people2.toArray(new String[people2.size()]));
        }
        if (Build.VERSION.SDK_INT >= 20) {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            if (b.mInvisibleActions.size() > 0) {
                ...
                for (int i = 0; i < b.mInvisibleActions.size(); i++) {
                    listBundle.putBundle(Integer.toString(i), NotificationCompatJellybean.getBundleForAction((NotificationCompat.Action) b.mInvisibleActions.get(i)));
                }
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            Iterator it2 = b.mPersonList.iterator();
            while (it2.hasNext()) {
    ...
    private static List combineLists(List list, List list2) {
        ...
        ArraySet<String> people = new ArraySet<>(list.size() + list2.size());
        people.addAll(list);
        people.addAll(list2);

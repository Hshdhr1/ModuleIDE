...
public final class CertificatePinner {
    ...
    public void check(String hostname, List list) throws SSLPeerUnverifiedException {
        List<Pin> pins = findMatchingPins(hostname);
        if (!pins.isEmpty()) {
            if (this.certificateChainCleaner != null) {
                list = this.certificateChainCleaner.clean(list, hostname);
            }
            int certsSize = list.size();
            for (int c = 0; c < certsSize; c++) {
                X509Certificate x509Certificate = (X509Certificate) list.get(c);
            ...
            int certsSize2 = list.size();
            for (int c2 = 0; c2 < certsSize2; c2++) {
                X509Certificate x509Certificate2 = (X509Certificate) list.get(c2);
    ...
    static final class Pin {
        ...
        Pin(String pattern, String pin) {
            ...
            if (pattern.startsWith("*.")) {
                host = HttpUrl.parse("http://" + pattern.substring("*.".length())).host();
            } else {
